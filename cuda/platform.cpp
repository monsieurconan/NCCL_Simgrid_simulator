...
double latency = 1e-6;
// simgrid::xbt::Extension<simgrid::s4u::Host, cudaDeviceProp> cudaDeviceProp::EXTENSION_ID =
// simgrid::s4u::Host::extension_create<cudaDeviceProp>();
...
double cudaDeviceProp::get_speed() { return 1e12; }

int cudaDeviceProp::parallelisation_degree() { return concurrentKernels; }

simgrid::s4u::NetZone *create_simple_node(simgrid::s4u::NetZone *root, int i_node, int ncpus_cores,
                                          int ngpus, cudaDeviceProp gpu_prop,
                                          double cpu_to_gpu_bandwidth,
                                          double gpu_to_gpu_bandwidth) {
    ...
    auto routerlink = node->create_link("ir" + std::to_string(i_node), gpu_to_gpu_bandwidth)
                          ->set_latency(latency);
    ...
    for (int i = 0; i < ncpus_cores; ++i) {
        cpu_cores.push_back(
            node->create_host("n" + std::to_string(i_node) + "cpu" + std::to_string(i), cpu_speed));
        cpu_cores[i]->set_property("type", "cpu");
        node->add_route(cpu_cores[i]->get_netpoint(), router, nullptr, nullptr,
                        {routerlink_in_route});
    }
    for (int i = 0; i < ngpus; ++i) {
        gpus.push_back(node->create_host("n" + std::to_string(i_node) + "_gpu" + std::to_string(i),
                                         gpu_prop.get_speed()));
        ...
        for (int j = 0; j < ncpus_cores; ++j) {
            auto link = node->create_link("in" + std::to_string(i_node) + "cpu" +
        ...
        for (int j = 0; j < i; ++j) {
            auto link = node->create_link("in" + std::to_string(i_node) + "gpu" +
                                              std::to_string(j) + ":" + std::to_string(i),
                                          gpu_to_gpu_bandwidth)
                            ->set_latency(latency);
            simgrid::s4u::LinkInRoute link_in_route{link};
