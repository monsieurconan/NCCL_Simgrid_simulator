...
void reduction(ncclRedOp_t op, void *recvbuf, void *tmpbuf, size_t n, ncclDataType_t datatype,
               cudaStream_t stream) {
    auto execution = simgrid::cuda::CreateGpuExec(n); // todo : better model
...
ncclResult_t ncclAllReduce(const void *sendbuff, void *recvbuff, size_t count,
    ...
    // larger amount of elements, more complex algo
    // https://www.linkedin.com/pulse/nccl-allreduce-algorithm-ring-%E5%BD%A6%E6%B0%91-%E8%B4%BE
...
ncclResult_t ncclAllGather(const void *sendbuff, void *recvbuff, size_t sendcount,
    ...
    if (sendcount < comm->nranks()) {
        ...
        for (int i = 0; i < comm->nranks() - 1; ++i) {
            ...
            ncclRecv(nullptr /*recvbuff[sendcount*i*sizeof(datatype)]*/, sendcount, datatype, peer,
                     comm, stream);
        }
        ncclGroupEnd();
        return ncclSuccess;
...
ncclResult_t ncclAllToAll(const void *sendbuff, void *recvbuff, size_t sendcount, size_t recvcount,
                          ncclDataType_t sendtype, ncclDataType_t recvtype, ncclComm_t comm,
                          cudaStream_t stream) {
    ncclGroupStart();
    for (int r = 0; r < comm->nranks(); r++) {
...
ncclResult_t debugTest(cudaStream_t stream) {
    auto execution = simgrid::cuda::CreateGpuExec(1e8); // todo : better model
    stream->launch(execution);
    return ncclSuccess;
}
